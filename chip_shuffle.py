...
def launch():
    ...
    for i in range(len(mp4_list)):
        filter_parts.append(f"[{i}:v]setpts=PTS/1.4[v{i}]; [{i}:a]atempo=1.4,volume=2[a{i}]")
    ...
    for attempt in range(3):
        if source_path.exists() and source_path.stat().st_size > 50:
            ...
            break  # 文件有大小，退出函数
        print(f"Attempt {attempt + 1}: File '{source_path}' is empty or does not exist. Retrying in 3 seconds...")
        time.sleep(3)
